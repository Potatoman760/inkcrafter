...
import { dialogueFontFamily, splitDialogueLine } from "@/bundle/spec/bundle/gameDoc";
...
export class DialogueBox {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    const presentation = getBundle(scene).game.dialogue;
    const nameSize = Math.round(presentation.name.size * PlayerSettings.textScale);
    const textSize = Math.round(presentation.text.size * PlayerSettings.textScale);
    ...
    this.speakerText = scene.add.text(PAD, -16, "", {
      fontFamily: dialogueFontFamily(presentation.name),
    ...
    this.bodyText = scene.add.text(PAD, PAD + 8, "", {
      fontFamily: dialogueFontFamily(presentation.text),
  ...
  setLine(speaker: string, text: string): void {
    this.stopTyping();
    const line = splitDialogueLine(
      speaker,
      text,
      getBundle(this.scene).game.dialogue.separateNames,
    );
    this.speakerText.setText(line.speaker).setVisible(line.speaker.length > 0);
    this.bodyText.setY(
      line.speaker.length > 0
        ? Math.max(PAD + 8, this.speakerText.y + this.speakerText.height + 10)
        : PAD + 8,
    );
